...
VALUES 
    (?, CURRENT DATE, <3>, <1>, <2>, <4>, <5>, <6>, <7>, <8>, <9>, <10>, (SELECT e.ID_empleado FROM empleado e WHERE e.tipo_empleado = 'Administrador Comercial'), ?, (SELECT cli.ID_cliente FROM cliente cli WHERE cli.nombre = <11>));
```
