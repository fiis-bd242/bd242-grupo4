...
select * from cliente where ruc_dni like ? and correo like ?;
```
...
INSERT INTO Invitacion (Asunto_Invitacion, Fecha_Envio, Tiempo_Maximo, Direccion_Proyecto, Comentario, Estado_Invitacion, ID_cliente) values (?, CURRENT_DATE, ?, ?, ?, 'No revisado', ?) RETURNING ID_invitacion;

INSERT INTO Requerimiento(Descrip_requerimiento, ID_invitacion) VALUES (?,?) RETURNING ID_requerimiento;
```
...
FROM 
    Cliente cli
LEFT JOIN Invitacion inv ON cli.ID_cliente = inv.ID_cliente
LEFT JOIN Revision_tecnica rev ON rev.ID_invitacion = inv.ID_invitacion
WHERE 
...
FROM 
    Invitacion inv
LEFT JOIN 
    Cliente cli ON inv.ID_cliente = cli.ID_cliente
WHERE 
    inv.Estado_Invitacion = 'No revisado' 
    AND inv.ID_invitacion = ?;

SELECT re.ID_requerimiento, re.Descrip_requerimiento 
FROM requerimiento re 
LEFT JOIN invitacion inv ON re.id_invitacion = inv.id_invitacion 
WHERE inv.estado_invitacion = 'No revisado' 
AND inv.id_invitacion = ?;

SELECT 
    inf.ID_informe_stock,
...
2. Al presionar el boton "Si se participara" se creara una revision y se actualizara el estado de la invitacion.

```sql

INSERT INTO Revision_tecnica (Estado_Participacion, ID_empleado, ID_invitacion, ID_informe_stock)
VALUES ('Aceptado', (SELECT e.ID_empleado FROM Empleado e WHERE e.tipo_empleado = 'Administrador Comercial'),?, ?)
...
3. Al presionar el boton "No se participara" se creara una revision y se actualizara el estado de la invitacion.

```sql

INSERT INTO Revision_tecnica (Estado_Participacion, ID_empleado, ID_invitacion, ID_informe_stock)
VALUES ('Rechazado', (SELECT e.ID_empleado FROM Empleado e WHERE e.tipo_empleado = 'Administrador Comercial'),?, ?)
RETURNING ID_revision_tecnica;
...
(Fecha_Presentacion, Precio_Propuesto, Descripcion_Tecnica, Descripcion_Economica, Calidad_Ofrecida, Seguridad_Ofrecida, Condicion_Pago, Tiempo_Ejecucion, Observacion_Propuesta, Estado_Propuesta, ID_empleado, ID_revision_tecnica, ID_cliente) 
VALUES
(CURRENT_DATE, ?, ?, ?, ?, ?, ?, ?, ?, 'No revisado', 
(SELECT e.ID_empleado FROM empleado e WHERE e.tipo_empleado = 'Administrador Comercial'), ?, ?)
RETURNING ID_presentacion_propuesta;

INSERT INTO Garantia (Descrip_garantia, ID_presentacion_propuesta) 
VALUES 
(?, ?)
RETURNING ID_garantia;

INSERT INTO Beneficio (Descrip_beneficio, ID_presentacion_propuesta) 
VALUES 
(?, ?)
...
SELECT pro.ID_presentacion_propuesta,
...
FROM Presentacion_propuesta pro
LEFT JOIN cliente cli ON pro.id_cliente = cli.id_cliente 
...
AND rev.Estado_Participacion = 'Aceptado' 
AND pro.estado_propuesta = 'No revisado'
AND cli.id_cliente = ?;

SELECT ga.ID_garantia, ga.Descrip_garantia 
FROM garantia ga
LEFT JOIN presentacion_propuesta pro ON ga.id_presentacion_propuesta = pro.id_presentacion_propuesta 
LEFT JOIN Revision_tecnica rev ON pro.ID_revision_tecnica = rev.ID_revision_tecnica 
WHERE rev.Estado_Participacion = 'Aceptado' 
AND pro.estado_propuesta = 'No revisado' 
AND pro.ID_presentacion_propuesta = ?;

SELECT be.ID_beneficio, be.Descrip_beneficio 
FROM beneficio be
LEFT JOIN presentacion_propuesta pro ON be.id_presentacion_propuesta = pro.id_presentacion_propuesta 
LEFT JOIN Revision_tecnica rev ON pro.ID_revision_tecnica = rev.ID_revision_tecnica 
WHERE rev.Estado_Participacion = 'Aceptado' 
AND pro.estado_propuesta = 'No revisado' 
AND pro.ID_presentacion_propuesta = ?;
...
INSERT INTO Adjudicacion (Fecha_Adjudicacion, Estado_Adjudicacion, ID_presentacion_propuesta, ID_cliente)
...
|Imagen Interfaz| ![image](https://github.com/user-attachments/assets/61d886e7-2561-4c3d-a94c-89318f09eeaf)|
...
```sql
SELECT
    r.Descripcion AS descripción_del_recurso, 
    l.Cantidad_total AS cantidad_del_lote, 
    l.ID_Lote AS id_lote, 
    r.Unidad AS unidad_de_medida, 
    rve.Estado AS estado_verificación
FROM
    Recurso r
JOIN
...
|Imagen Interfaz| ![image](https://github.com/user-attachments/assets/bd5389c4-5a17-400c-9719-4e913bf7cece)|

Eventos:

1. Seleccionar Boton de Si o No en Certificado de Calidad:

- Si se escoge si (1):
```sql
UPDATE Registro_Verificacion
SET Cumple_certificado_calidad = True
WHERE ID_registro_verificacion = (
    SELECT l.ID_registro_verificacion
    FROM Lote l
    WHERE l.ID_Lote = ?
);
...
```sql
UPDATE Registro_Verificacion
SET Cumple_certificado_calidad = False
WHERE ID_registro_verificacion = (
    SELECT l.ID_registro_verificacion
    FROM Lote l
 WHERE l.ID_Lote = ?
...
```
1. Seleccionar Boton de Si o No en Ficha tecnica:

- Si se escoge si (3):
```sql
UPDATE Registro_Verificacion
SET Cumple_ficha_tecnica = True
WHERE ID_registro_verificacion = (
    SELECT l.ID_registro_verificacion
    FROM Lote l
    WHERE l.ID_Lote = ?
...
```
3. Se agregan las observaciones (5):
```sql
UPDATE Registro_Verificacion
SET Observaciones = 'Se detectaron irregularidades menores'
WHERE ID_registro_verificacion = (
    SELECT ID_registro_verificacion 
    FROM Lote
    WHERE ID_Lote = 'L00103'
);
```
**Caso 3**
...
| -------------------- | ----------------------------------------------------------------------------------------- |
| Codigo Interfaz      | I503                                                                                      |
| Imagen Interfaz      | ![image](https://github.com/user-attachments/assets/f9fcdff3-0d08-4aff-a0df-7028c1444124) |

Evento:

Boton Aprobado
```sql
UPDATE Registro_Verificacion_Estado rve
SET Estado = 'Aprobado'
FROM Registro_Verificacion rv
JOIN Lote l ON rv.ID_registro_verificacion = l.ID_registro_verificacion
WHERE rve.ID_registro_verificacion = rv.ID_registro_verificacion
  AND l.ID_Lote = ?
  AND rve.Estado = 'Aprobado';

```
Boton Rechazado
```sql
UPDATE Registro_Verificacion_Estado rve
SET Estado = 'No Aprobado'
FROM Registro_Verificacion rv
JOIN Lote l ON rv.ID_registro_verificacion = l.ID_registro_verificacion
WHERE rve.ID_registro_verificacion = rv.ID_registro_verificacion
  AND l.ID_Lote = ?
  AND rve.Estado = 'No Aprobado';
...
```sql
SELECT
    r.Descripcion AS descripción_recurso, 
    l.Cantidad_total AS cantidad_del_lote, 
    l.ID_Lote AS lote, 
    r.Unidad AS unidad_de_medida, 
    ie.Estado AS estado_inspección
FROM
    Recurso r
JOIN
    Lote l ON r.ID_lote = l.ID_Lote
...
1. Boton filtro
2. Boton Nuevo: Lleva a la Interfaz I505
3. Barra de búsqueda
...
| Imagen Interfaz      | ![image](https://github.com/user-attachments/assets/8578e7d2-d47d-4d69-b60a-8523e200a386) |

Evento:

1. Boton Control Dimensional: Lleva a la Interfaz I506
2. Boton Control de composicion: Lleva a la Interfaz I507

**Caso 6**
| Codigo Requerimiento | R506                            |
| -------------------- | ------------------------------- |
| Codigo Interfaz      | I506                            |
| Imagen Interfaz      | ![image](https://github.com/user-attachments/assets/73477619-d33e-4a1d-856e-8665a3eae164)|
Eventos:
1. 
```sql
INSERT INTO Inspeccion (ID_inspeccion, Fecha_inspeccion, Observaciones, ID_empleado, ID_resultado_inspeccion) 
VALUES (?, ?, ?, ?, ?);

INSERT INTO Control_dimensional (ID_inspeccion, Dimensiones, Tolerancias, Planitud, Rectitud, Unidad_de_medida, Metodo_medicion)
VALUES (?, ?, ?, ?, ?, ?, ?);
```
2. Boton Siguiente
...
| Imagen Interfaz      | ![image](https://github.com/user-attachments/assets/3cb6623a-e996-494c-aefd-b6b2e847fe5f)|

Eventos:
1. 
```sql
INSERT INTO Inspeccion (ID_inspeccion, Fecha_inspeccion, Observaciones, ID_empleado, ID_resultado_inspaccion) 
VALUES (?, ?, ?, ?, ?);

INSERT INTO Control_de_Composicion (ID_inspeccion, Componente_quimico, Concentracion_medida, Temperatura_prueba, Metodo_Analisis)
VALUES (?, ?, ?, ?, ?);
```
2. Boton Siguiente: Lleva a la Interfaz I508

**Caso 8**
| Codigo Requerimiento | R508                            |
| -------------------- | ------------------------------- |
| Codigo Interfaz      | I508                            |
| Imagen Interfaz      |![image](https://github.com/user-attachments/assets/942b796f-dc35-4f51-91d5-ec5c58cb5c4a)|

Boton Aprobado
```sql
UPDATE Inspeccion_Estado
SET Estado = 'Aprobado'
FROM Inspeccion
JOIN Lote ON Lote.ID_inspeccion = Inspeccion.ID_inspeccion
WHERE Inspeccion_Estado.ID_inspeccion = Inspeccion.ID_inspeccion
  AND Lote.ID_Lote = ?
  AND Inspeccion_Estado.Estado = 'Aprobado';
...
| Imagen Interfaz      |![image](https://github.com/user-attachments/assets/9d8af5a7-0b33-4bee-846a-dba9cd7cd48f)|
...
WHERE
	...
	AND l.ID_Lote = ?;
```
3. Boton nuevo: Lleva a la Interfaz I510
...
| Imagen Interfaz      |![image](https://github.com/user-attachments/assets/5d63ffd7-e223-4e04-92c1-dfd12d62c814)|

1. Boton aprobado:
```sql
UPDATE Resultado_Inspeccion_Estado_Final
SET Estado_Final = 'Aprobado'
FROM Resultado_Inspeccion
JOIN Inspeccion ON Inspeccion.ID_resultado_inspeccion = Resultado_Inspeccion.ID_resultado_inspeccion
JOIN Lote ON Lote.ID_inspeccion = Inspeccion.ID_inspeccion
WHERE Resultado_Inspeccion_Estado_Final.ID_resultado_inspeccion = Resultado_Inspeccion.ID_resultado_inspeccion
  AND Lote.ID_Lote = ?
  AND Resultado_Inspeccion_Estado_Final.Estado_Final != 'Aprobado';
```
2. Boton No Conformidad:
```sql
UPDATE Resultado_Inspeccion_Estado_Final
SET Estado_Final = 'No Conformidad'
FROM Resultado_Inspeccion
JOIN Inspeccion ON Inspeccion.ID_resultado_inspeccion = Resultado_Inspeccion.ID_resultado_inspeccion
JOIN Lote ON Lote.ID_inspeccion = Inspeccion.ID_inspeccion
WHERE Resultado_Inspeccion_Estado_Final.ID_resultado_inspeccion = Resultado_Inspeccion.ID_resultado_inspeccion
  AND Lote.ID_Lote = ?
  AND Resultado_Inspeccion_Estado_Final.Estado_Final != 'No Conformidad';
```

3. Agregar Observaciones finales:

INSERT INTO Resultado_inspeccion (ID_resultado_inspeccion, Observaciones, Fecha_resultado, ID_doc_no_conformidad) 
VALUES (?, ?, ?, ?)
