...
```sql
SELECT
    r.ID_recurso AS "Código Recurso",
...
LEFT JOIN
    Inspeccion i ON r.ID_inspeccion = i.ID_inspeccion
LEFT JOIN
...
JOIN
    Lote l ON r.ID_lote = l.ID_lote
LEFT JOIN
    Inspeccion i ON i.ID_inspeccion = r.ID_inspeccion
LEFT JOIN
    Inspeccion_Estado ie ON i.ID_inspeccion = ie.ID_inspeccion
WHERE
    CASE 
        WHEN '<FiltroSeleccionado>' = 'CodigoRecurso' THEN r.ID_recurso
        WHEN '<FiltroSeleccionado>' = 'DescripcionRecurso' THEN r.Descripcion
        WHEN '<FiltroSeleccionado>' = 'CantidadLote' THEN l.Cantidad_total
        WHEN '<FiltroSeleccionado>' = 'Lote' THEN l.ID_lote
        WHEN '<FiltroSeleccionado>' = 'UnidadMedida' THEN r.Unidad
        WHEN '<FiltroSeleccionado>' = 'EstadoInspeccion' THEN ie.Estado
        ELSE NULL
    END LIKE '%' || COALESCE('<Busqueda>', '') || '%'
ORDER BY
    l.ID_lote;
...
WHERE
    CASE 
        WHEN '<ColumnaSeleccionada>' = 'Descripcion_Producto' THEN r.Descripcion LIKE '%<Busqueda>%'
        WHEN '<ColumnaSeleccionada>' = 'ID_Lote' THEN l.ID_lote LIKE '%<Busqueda>%'
        WHEN '<ColumnaSeleccionada>' = 'Estado_Insp' THEN ie.Estado LIKE '%<Busqueda>%'
        ELSE 1 = 1  -- Si no se elige una columna o el valor es incorrecto, muestra todos
    END
ORDER BY
    l.ID_lote;
```
...
VALUES (?, ?, 'Observación de ejemplo', ?, ?);

INSERT INTO Control_dimensional (ID_inspeccion, Dimensiones, Tolerancias, Planitud, Rectitud)
VALUES (?, 30.0, 0.4, 0.2, 0.3);
```
2. Boton Siguiente
**Caso 5**
...
1. 
```sql
INSERT INTO Resultado_inspeccion (ID_Inspeccion, Analisis_Quimico, Comparacion_Especificaciones, Observaciones)
VALUES (?, ?, ?, ?);
```
2. Boton Siguiente
